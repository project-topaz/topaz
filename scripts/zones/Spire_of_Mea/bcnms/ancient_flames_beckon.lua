...
local function otherLights(player)
    return (player:hasKeyItem(tpz.ki.LIGHT_OF_DEM)   and 1 or 0) +
           (player:hasKeyItem(tpz.ki.LIGHT_OF_HOLLA) and 1 or 0)
end
...
function onBattlefieldLeave(player, battlefield, leavecode)
    if leavecode == tpz.battlefield.leaveCode.WON then
        ...
        player:startEvent(32001, battlefield:getArea(), clearTime, partySize, battlefield:getTimeInside(), player:getZoneID(), 0, battlefield:getLocalVar("[cs]bit"), arg8)
...
function onEventFinish(player, csid, option)
    if csid == 32001 then
        local teleportTo = tpz.teleport.id.EXITPROMMEA
        local ki = tpz.ki.LIGHT_OF_MEA
