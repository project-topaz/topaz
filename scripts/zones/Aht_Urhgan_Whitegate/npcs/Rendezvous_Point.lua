    ...
    if player:getQuestStatus(JEUNO,tpz.quest.id.jeuno.MIRROR_MIRROR) == QUEST_COMPLETED then
        if player:getQuestStatus(JEUNO,tpz.quest.id.jeuno.CHAMELEON_CAPERS) == QUEST_COMPLETED then
            if player:hasItem(1820) and (bit.band(optionsMask, bit.lshift(1,5)) == 0) then -- T.M. Fortitude
                player:startEvent(965, zone, 1820, 0, 0, 0, styleParam, lookParam, fellowParam)
            elseif player:hasItem(1839) and (bit.band(optionsMask, bit.lshift(1,6)) == 0) then -- T.M. Might
                player:startEvent(965, zone, 1839, 0, 0, 0, styleParam, lookParam, fellowParam)
            elseif player:hasItem(1876) and (bit.band(optionsMask, bit.lshift(1,7)) == 0) then -- T.M. Endurance
                player:startEvent(965, zone, 1876, 0, 0, 0, styleParam, lookParam, fellowParam)
            else
                player:startEvent(966, zone, menuParam, 0, 0, pearl, styleParam, lookParam, fellowParam)
            end
        else
            player:startEvent(966, zone, menuParam, 0, 0, pearl, styleParam, lookParam, fellowParam)
        end
...
-- TODO: Quest options as quests are implemented
    ...
    elseif csid == 966 and (option == 0xA000009 or option == 0x5000009) then -- combat signals
