...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 378 then
        if npcUtil.completeQuest(
            ...
            {xp = 200, item = 13093, title=tpz.title.STAR_ONION_BRIGADE_MEMBER, fame=10})
        then
    ...
    elseif csid == 400 then
        if npcUtil.completeQuest(
            ...
            {xp = 700, item = 4857, title=tpz.title.SOB_SUPER_HERO, var="KnowOnesOnions", fame=10})
        then
            player:tradeComplete()
    ...
    elseif csid == 421 then
        if npcUtil.completeQuest(
            ...
            {xp = 1200, item = 13204, title=tpz.title.FAKEMOUSTACHED_INVESTIGATOR, fame=10})
        then
