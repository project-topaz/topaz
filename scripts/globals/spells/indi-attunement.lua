...
function onMagicCastingCheck(caster, target, spell)
    if target:hasStatusEffect(tpz.effect.COLURE_ACTIVE) then
        ...
        if effect:getSubType() ==  tpz.effect.GEO_MAGIC_EVASION_BOOST then
...
function onSpellCast(caster, target, spell)
    tpz.geo.doIndiSpell(caster, target, spell, tpz.effect.GEO_MAGIC_EVASION_BOOST, tpz.allegiance.PLAYER)
