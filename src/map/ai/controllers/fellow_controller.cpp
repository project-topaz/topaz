...
CFellowController::CFellowController(CCharEntity* PChar, CFellowEntity* PFellow) : CMobController(PFellow)
{
}
...
{
    if ((POwner->PMaster == nullptr || POwner->PMaster->isMounted() ||
        POwner->PMaster->isDead()) && POwner->isAlive())
    {
        POwner->Die();
        return;
    }
