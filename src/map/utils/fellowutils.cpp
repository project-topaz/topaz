...
namespace fellowutils
{
    ...
    uint16 GetBase(CFellowEntity* PFellow, uint8 rank)
     {
        ...
        if (lvl > 50)
        {
            switch(rank)
            {
                case 1: return (uint16)(153 + (lvl - 50) * 5.0f); // A
                case 2: return (uint16)(147 + (lvl - 50) * 4.9f); // B
                case 3: return (uint16)(136 + (lvl - 50) * 4.8f); // C
                case 4: return (uint16)(126 + (lvl - 50) * 4.7f); // D
                case 5: return (uint16)(116 + (lvl - 50) * 4.5f); // E
                case 6: return (uint16)(106 + (lvl - 50) * 4.4f); // F
                case 7: return (uint16)(96 + (lvl - 50) * 4.3f); // G
            }
        }
        else
        {
            switch (rank)
            {
                case 1: return (uint16)(6 + (lvl - 1) * 3.0f);
                case 2: return (uint16)(5 + (lvl - 1) * 2.9f);
                case 3: return (uint16)(5 + (lvl - 1) * 2.8f);
                case 4: return (uint16)(4 + (lvl - 1) * 2.7f);
                case 5: return (uint16)(4 + (lvl - 1) * 2.5f);
                case 6: return (uint16)(3 + (lvl - 1) * 2.4f);
                case 7: return (uint16)(3 + (lvl - 1) * 2.3f);
            }
        }
    ...
    void SpawnFellow(CCharEntity* PMaster, uint32 FellowID, bool spawningFromZone)
    {
        ...
        if (PFellow)
        {
            ...
            {
                Sql_Query(SqlHandle,
                "UPDATE char_fellow SET kills = 0, maxTime = %u WHERE charid = %u",
                GetMaxTime(PMaster), PMaster->id);
                if (PMaster->GetLocalVar("triggerFellow") == 0)
                    PMaster->pushPacket(new CMessageSpecialPacket(PFellow,
                    GetMessageOffset(PMaster->getZone()) + FELLOWMESSAGEOFFSET_CALL +
                    GetPersonalityOffset(PMaster)));
            }
        }
        else
        {
    ...
    uint16 GetMaxTime(CCharEntity* PChar)
    {
        ...
        if (Sql_Query(SqlHandle, QueryP, PChar->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 && Sql_NextRow(SqlHandle) == SQL_SUCCESS)
                bond = (uint8)Sql_GetIntData(SqlHandle, 0);
        if (bond >= 120) maxTime = 90 * 60; // 90 min
        else if (bond >= 100) maxTime = 85 * 60; // 85 min
        else if (bond >= 80) maxTime = 80 * 60; // 80 min
        else if (bond >= 70) maxTime = 75 * 60; // 75 min
        else if (bond >= 60) maxTime = 70 * 60; // 70 min
        else if (bond >= 50) maxTime = 65 * 60; // 65 min
        else if (bond >= 40) maxTime = 60 * 60; // 60 min
        else if (bond >= 20) maxTime = 55 * 60; // 55 min
        else if (bond >= 10) maxTime = 50 * 60; // 50 min
    ...
    CFellowEntity* LoadFellow(CCharEntity* PMaster, uint32 FellowID, bool spawningFromZone)
    {
        ...
        const char* QueryFace =
            "SELECT char_fellow.face FROM char_fellow WHERE\
            charid = %u";
        if (Sql_Query(SqlHandle, QueryFace, PMaster->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 &&
            Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            PFellow->look.face = (uint8)Sql_GetIntData(SqlHandle, 0);
        }
        const char* QueryMain =
            "SELECT item_equipment.MId FROM item_equipment, char_fellow WHERE\
            char_fellow.main = item_equipment.itemId AND\
            charid = %u";
        if (Sql_Query(SqlHandle, QueryMain, PMaster->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 &&
            Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
        ...
        const char* QuerySub =
            "SELECT item_equipment.MId, item_weapon.skill FROM char_fellow LEFT JOIN\
        ...
        if (Sql_Query(SqlHandle, QuerySub, PMaster->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 &&
            Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
        ...
        else if ((uint16)Sql_GetIntData(SqlHandle, 1) == SKILL_GREAT_SWORD ||
            (uint16)Sql_GetIntData(SqlHandle, 1) == SKILL_GREAT_AXE ||
            (uint16)Sql_GetIntData(SqlHandle, 1) == SKILL_SCYTHE ||
            (uint16)Sql_GetIntData(SqlHandle, 1) == SKILL_POLEARM ||
            (uint16)Sql_GetIntData(SqlHandle, 1) == SKILL_GREAT_KATANA ||
            (uint16)Sql_GetIntData(SqlHandle, 1) == SKILL_STAFF)
        {
            PFellow->look.sub = 0;
        }
        uint8 personality = 0;
        const char* QueryP ="SELECT personality FROM char_fellow WHERE charid = %u";
        if (Sql_Query(SqlHandle, QueryP, PMaster->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 && Sql_NextRow(SqlHandle) == SQL_SUCCESS)
                personality = (uint8)Sql_GetIntData(SqlHandle, 0);
        const char* QueryHead =
            "SELECT fellow_headgear.%s FROM fellow_headgear, char_fellow WHERE\
            char_fellow.head = fellow_headgear.rank AND\
            charid = %u";
        if (Sql_Query(SqlHandle, QueryHead, std::to_string(personality), PMaster->id) !=
            SQL_ERROR && Sql_NumRows(SqlHandle) != 0 && Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            PFellow->look.head = (uint16)Sql_GetIntData(SqlHandle, 0) + 0x1000;
        }
        const char* QueryBody =
            "SELECT fellow_armor.body FROM fellow_armor, char_fellow WHERE\
            char_fellow.body = fellow_armor.rank AND\
            charid = %u";
        if (Sql_Query(SqlHandle, QueryBody, PMaster->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 &&
            Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            PFellow->look.body = (uint16)Sql_GetIntData(SqlHandle, 0) + 0x2000;
        }
        const char* QueryHands =
            "SELECT fellow_armor.hands FROM fellow_armor, char_fellow WHERE\
            char_fellow.hands = fellow_armor.rank AND\
            charid = %u";
        if (Sql_Query(SqlHandle, QueryHands, PMaster->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 &&
            Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            PFellow->look.hands = (uint16)Sql_GetIntData(SqlHandle, 0) + 0x3000;
        }
        const char* QueryLegs =
            "SELECT fellow_armor.legs FROM fellow_armor, char_fellow WHERE\
            char_fellow.legs = fellow_armor.rank AND\
            charid = %u";
        if (Sql_Query(SqlHandle, QueryLegs, PMaster->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 &&
            Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            PFellow->look.legs = (uint16)Sql_GetIntData(SqlHandle, 0) + 0x4000;
        }
        const char* QueryFeet =
            "SELECT fellow_armor.feet FROM fellow_armor, char_fellow WHERE\
            char_fellow.feet = fellow_armor.rank AND\
            charid = %u";
        if (Sql_Query(SqlHandle, QueryFeet, PMaster->id) != SQL_ERROR &&
            Sql_NumRows(SqlHandle) != 0 &&
            Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
    ...
    //      PFellow->SetSLevel(PMaster->GetMLevel() / 2); // temp to test scaling
            ...
            switch (type)
            {
            case 1: job = 7; sjob = 1; break; // Shield (PLD/WAR)
            case 2: job = 1; sjob = 2; break; // Attacker (WAR/MNK)
            case 3: job = 3; sjob = 1; break; // Healer (WHM/WAR)
            case 4: job = 7; sjob = 1; break; // Stalwart (PLD/WAR)
            case 5: job = 1; sjob = 2; break; // Fierce (WAR/MNK)
            case 6: job = 3; sjob = 1; break; // Soothing (WHM/WAR)
            }
            PFellow->SetMJob(job);
            ...
            if (Sql_Query(SqlHandle, Query3, PMaster->id) != SQL_ERROR &&
                Sql_NumRows(SqlHandle) != 0 &&
                Sql_NextRow(SqlHandle) == SQL_SUCCESS)
            {
    ...
    uint32 GetExpNEXTLevel(uint8 charlvl)
    {
        ...
        switch (charlvl)
        {
            case 30: expNEXTLvl = 5800; break;
            case 31: expNEXTLvl = 5900; break;
            case 32: expNEXTLvl = 6000; break;
            case 33: expNEXTLvl = 6100; break;
            case 34: expNEXTLvl = 6200; break;
            case 35: expNEXTLvl = 6300; break;
            case 36: expNEXTLvl = 6400; break;
            case 37: expNEXTLvl = 6500; break;
            case 38: expNEXTLvl = 6600; break;
            case 39: expNEXTLvl = 6700; break;
            case 40: expNEXTLvl = 6800; break;
            case 41: expNEXTLvl = 6900; break;
            case 42: expNEXTLvl = 7000; break;
            case 43: expNEXTLvl = 7100; break;
            case 44: expNEXTLvl = 7200; break;
            case 45: expNEXTLvl = 7300; break;
            case 46: expNEXTLvl = 7400; break;
            case 47: expNEXTLvl = 7500; break;
            case 48: expNEXTLvl = 7600; break;
            case 49: expNEXTLvl = 7700; break;
            case 50: expNEXTLvl = 7800; break;
            case 51: expNEXTLvl = 8000; break;
            case 52: expNEXTLvl = 9200; break;
            case 53: expNEXTLvl = 10400; break;
            case 54: expNEXTLvl = 11600; break;
            case 55: expNEXTLvl = 12800; break;
            case 56: expNEXTLvl = 14000; break;
            case 57: expNEXTLvl = 15200; break;
            case 58: expNEXTLvl = 16400; break;
            case 59: expNEXTLvl = 17600; break;
            case 60: expNEXTLvl = 18800; break;
            case 61: expNEXTLvl = 20000; break;
            case 62: expNEXTLvl = 21500; break;
            case 63: expNEXTLvl = 23000; break;
            case 64: expNEXTLvl = 24500; break;
            case 65: expNEXTLvl = 26000; break;
            case 66: expNEXTLvl = 27500; break;
            case 67: expNEXTLvl = 29000; break;
            case 68: expNEXTLvl = 30500; break;
            case 69: expNEXTLvl = 32000; break;
            case 70: expNEXTLvl = 34000; break;
            case 71: expNEXTLvl = 36000; break;
            case 72: expNEXTLvl = 38000; break;
            case 73: expNEXTLvl = 40000; break;
            case 74: expNEXTLvl = 42000; break;
            case 75: expNEXTLvl = 44000; break;
        }
    ...
    void DistributeExperiencePoints(CFellowEntity* PFellow, CMobEntity* PMob, CCharEntity* PChar)
    {
        ...
        {
            if (MLvl - FLvl < 0) exp = std::clamp(((MLvl - FLvl) * 15) + 100, 0, 85);
            else if (MLvl - FLvl == 0) exp = 100;
            else if (MLvl - FLvl == 1) exp = 120;
            else if (MLvl - FLvl == 2) exp = 140;
            else if (MLvl - FLvl == 3) exp = 160;
            else if (MLvl - FLvl >= 4) exp = 200;
        }
        else if (FLvl > 30 && FLvl <= 50)
        {
            if (MLvl - FLvl < 0) exp = std::clamp(((MLvl - FLvl) * 15) + 100, 0, 85);
            else if (MLvl - FLvl == 0) exp = 100;
            else if (MLvl - FLvl == 1) exp = 125;
            else if (MLvl - FLvl == 2) exp = 150;
            else if (MLvl - FLvl >= 3) exp = 200;
        }
        else if (FLvl > 50 && FLvl <= 55)
        {
            ...
            else if (MLvl - FLvl >= 4) exp = 250;
        }
        else if (FLvl > 55 && FLvl <= 60)
        {
            if (MLvl - FLvl < 0) exp = std::clamp(((MLvl - FLvl) * 15) + 100, 0, 85);
            else if (MLvl - FLvl == 0) exp = 100;
            else if (MLvl - FLvl == 1) exp = 130;
            else if (MLvl - FLvl == 2) exp = 160;
            else if (MLvl - FLvl == 3) exp = 200;
            else if (MLvl - FLvl == 4) exp = 240;
            else if (MLvl - FLvl >= 5) exp = 250;
        }
        else if (FLvl > 60)
        {
            if (MLvl - FLvl < 0) exp = std::clamp(((MLvl - FLvl) * 15) + 100, 0, 85);
            ...
            else if (MLvl - FLvl == 5) exp = 280;
            else if (MLvl - FLvl >= 6) exp = 300;
        }
    ...
    void AddKillCount(CCharEntity* PMaster)
    {
            ...
            Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            ...
            uint16 maxKills = 15;
            if (fellowBond >= 120) maxKills = 30;
            else if (fellowBond >= 70) maxKills = 25;
            else if (fellowBond >= 30) maxKills = 20;
    ...
    uint8 GetPersonalityOffset(CCharEntity* PChar)
    {
        ...
        if (Sql_Query(SqlHandle, QueryP, PChar->id) != SQL_ERROR &&
            ...
            switch (personality)
            {
                case 4: personality = 0; break;
                case 8: personality = 1; break;
                case 12: personality = 2; break;
                case 16: personality = 3; break;
                case 40: personality = 4; break;
                case 44: personality = 5; break;
                case 20: personality = 7; break;
                case 24: personality = 8; break;
                case 28: personality = 9; break;
                case 32: personality = 10; break;
                case 36: personality = 11; break;
                case 48: personality = 12; break;
            }
        return personality;
    }
...
// 0-[chat(rnd 1-3)] 1-[Kills if > 0] 2-[MP status] 3-[HP status] 4-[EXP status if not capped]
            switch (chatCounter)
            {
                case 0:
                        switch(roll)
                        {
                            case 0: message = FELLOWMESSAGEOFFSET_GENERAL_CHAT; break;
                            case 1: message = FELLOWMESSAGEOFFSET_GENERAL_CHAT2; break;
                            case 2: message = FELLOWMESSAGEOFFSET_GENERAL_CHAT3; break;
                        }
                        if (kills > 0)
