...
void LoadTrustStatsAndSkills(CTrustEntity* PTrust)
{
    ...
    PTrust->stats.STR = (uint16)std::floor((fSTR + mSTR + sSTR) * 0.85);
    PTrust->stats.DEX = (uint16)std::floor((fDEX + mDEX + sDEX) * 0.85);
    PTrust->stats.VIT = (uint16)std::floor((fVIT + mVIT + sVIT) * 0.85);
    PTrust->stats.AGI = (uint16)std::floor((fAGI + mAGI + sAGI) * 0.85);
    PTrust->stats.INT = (uint16)std::floor((fINT + mINT + sINT) * 0.85);
    PTrust->stats.MND = (uint16)std::floor((fMND + mMND + sMND) * 0.85);
    PTrust->stats.CHR = (uint16)std::floor((fCHR + mCHR + sCHR) * 0.85);
