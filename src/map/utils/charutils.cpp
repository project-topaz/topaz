    ...
    void DelExperiencePoints(CCharEntity* PChar, float retainPercent, uint16 forcedXpLoss)
    {
        ...
        if (PChar->jobs.exp[PChar->GetMJob()] < exploss)
        {
            if (PChar->jobs.job[PChar->GetMJob()] > 1)
            {
                ...
                PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF, new CMessageFinishPacket(PChar, PChar, PChar->jobs.job[PChar->GetMJob()], 0, 11));
    ...
    void AddExperiencePoints(bool expFromRaise, CCharEntity* PChar, CBaseEntity* PMob, uint32 exp, EMobDifficulty mobCheck, bool isexpchain)
    {
        ...
        if (!expFromRaise && exp > 0)
        {
            if (mobCheck >= EMobDifficulty::EvenMatch && isexpchain)
            {
                ...
                {
                    if (onLimitMode)
                        PChar->pushPacket(new CMessageFinishPacket(PChar, PChar, exp, PChar->expChain.chainNumber, 372));
                    else
                        PChar->pushPacket(new CMessageFinishPacket(PChar, PChar, exp, PChar->expChain.chainNumber, 253));
                }
                else
                {
                    if (onLimitMode)
                    {
                        PChar->pushPacket(new CMessageFinishPacket(PChar, PChar, exp, 0, 371));
                    }
                    else
                    {
                        PChar->pushPacket(new CMessageFinishPacket(PChar, PChar, exp, 0, 8));
                    }
                }
        ...
        if (onLimitMode)
        {
            ...
            {
                PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF, new CMessageFinishPacket(PChar, PMob, PChar->PMeritPoints->GetMeritPoints(), 0, 50));
            }
        }
        else
        {
        ...
        if ((currentExp + exp) >= GetExpNEXTLevel(PChar->jobs.job[PChar->GetMJob()]) && !onLimitMode)
        {
            ...
            else
            {
                ...
                PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF, new CMessageFinishPacket(PChar, PMob, PChar->jobs.job[PChar->GetMJob()], 0, 9));
                PChar->pushPacket(new CCharStatsPacket(PChar));
